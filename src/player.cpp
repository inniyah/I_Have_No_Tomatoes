...
void draw_player_icons() {
	...
	if(p1.alive && p_icon_alpha[0]) {
		...
		if(p1.jumping) {
			...
			if(map_data[p1.jump_tx][p1.jump_ty][1] && jy < TILE_H && p1.jump_pos > 0.5f)
				jy = TILE_H;
	...
	if(p2.alive && p_icon_alpha[1]) {
		...
		if(p2.jumping) {
			...
			// If we're jumping to a block, make sure we don't "sink" in it
			if(map_data[p2.jump_tx][p2.jump_ty][1] && jy < TILE_H && p2.jump_pos > 0.5f)
				jy = TILE_H;
...
void PLAYER::move() {
	...
	if(jumping) {
		...
		if(map_data[jump_tx][jump_ty][1] && jump_pos > 0.9f) {
...
void PLAYER::draw() {
	...
	if(jumping) {
		...
		if(map_data[jpox][jpoy][1])
			shadow_offset = TILE_H + 0.01f;
		else if(!map_data[jpox][jpoy][0])
	...
	if(jumping) {
		...
		// If we're jumping to a block, make sure we don't "sink" in it
		if(map_data[jump_tx][jump_ty][1] && jy < TILE_H && jump_pos > 0.5f)
			jy = TILE_H;
