	...
	for(int y=0; y<MAP_H; y++) {
		for(int x=0; x<MAP_W; x++) {
			...
			if(map_data[x][y][1]) {
				...
				// Left
				BIND_TEXTURE(maptex[map_data[x][y][1] - 1]);
				glColor3f(1,1,1);
			...
			// Draw the lower tile
				...
				BIND_TEXTURE(maptex[map_data[x][y][0] - 1]);
				glBegin(GL_TRIANGLE_STRIP);
					glNormal3f(0,1,0);
...
void save_map(char *file) {
	...
	for(int y=0; y < MAP_H; y++) {
		for(int x=0; x < MAP_W; x++) {
			...
			fputc(map_data[x][y][0], f);
			fputc(map_data[x][y][1], f);
...
void load_map(char *file) {
	...
	for(int y=0; y < MAP_H; y++) {
		for(int x=0; x < MAP_W; x++) {
			...
			map_data[x][y][0] = fgetc(f);
			map_data[x][y][1] = fgetc(f);
